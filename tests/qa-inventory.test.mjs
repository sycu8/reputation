...
test("D11/D12 dashboard surfaces expose feedback, billing, admin, reports", async () => {
  ...
  assert.match(js, /readableMentionBody/);
  assert.match(js, /mentionBodyHtml/);
  assert.match(js, /mentionTitle/);
  assert.match(css, /\.mention-body/);
  ...
  assert.match(js, /applySignedInChrome/);
  assert.match(js, /pulsewatch-session-email/);
  assert.match(css, /panelIn/);
  assert.match(css, /is-entering-app/);
...
test("marketing landing page uses PulseWatch brand guide surfaces", async () => {
  ...
  assert.match(html, /data-auth-email/);
  ...
  assert.match(css, /profile-chip/);
  assert.match(css, /authSwap/);
  ...
  assert.match(js, /Optimistic signed-in chrome/);
  assert.doesNotMatch(html, /OrangeCloud Reputation/);
