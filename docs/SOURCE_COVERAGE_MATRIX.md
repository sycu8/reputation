...
| Facebook closed groups | Customer-authorized Meta group access only (member/admin token or approved product) | Official/authorized Graph access when entitled; **never** join-scrape or bypass invite walls | Capability dependent | group id, post/comment text, author where permitted | Closed/private groups require membership + platform approval. Fake “available” status is forbidden. | `contract-required` |
| Telegram private groups/channels | Bot token / client API for groups the customer administers or has invited the bot into | Official Bot/Client API with explicit invite | Capability dependent | chat id, message text, timestamps, sender where permitted | No global Telegram search. Only chats the workspace is authorized to read. | `contract-required` |
| Zalo groups | Official Zalo OA / partner APIs for communities the customer owns or is contracted to monitor | Official/partner API only | Capability dependent | group metadata and messages where entitlement allows | Do not scrape Zalo app UI or private invites. | `contract-required` |
| Discord private servers | Bot token with guild membership for servers the customer administers | Official Discord API | Capability dependent | channel/message content in authorized guilds | Bot must be invited; honor intent privileges and Discord ToS. | `contract-required` |
