...
| Reddit | Approved Data API / contracted access; optionally licensed provider | Official/contracted API only for production commercial monitoring | Depends on agreement; target 5-15 min if granted | posts/comments, subreddit, author, timestamps, scores, URLs | Reddit terms state commercial use may require a separate agreement. Do not design production around unauthenticated scraping. | `contract-required` |
