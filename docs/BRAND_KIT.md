...
| Technical key | Public label | Stripe payment link (in-app only) |
|---|---|---|
...
- Marketing homepage (`/`): showcase + CTAs to `/app/` only — **no** Stripe checkout links.
- After signup/login: Settings → Billing opens these Stripe Payment Links.
